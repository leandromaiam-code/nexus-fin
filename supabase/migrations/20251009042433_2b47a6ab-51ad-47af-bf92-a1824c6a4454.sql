-- Corrigir a view materializada mv_family_budget_performance para incluir TODAS as transações
...
family_spending AS (
  ...
  GROUP BY mf.familia_id, DATE_TRUNC('month', t.transaction_date)::date, t.category_id, c.name, c.icon_name
)
SELECT 
  ROW_NUMBER() OVER (ORDER BY COALESCE(fb.familia_id, fs.familia_id), COALESCE(fb.month, fs.month), COALESCE(fb.category_id, fs.category_id)) as id,
  COALESCE(fb.familia_id, fs.familia_id) as familia_id,
  COALESCE(fb.month, fs.month) as month,
  COALESCE(fb.category_id, fs.category_id) as category_id,
  COALESCE(fb.category_name, fs.category_name) as category_name,
  COALESCE(fb.icon_name, fs.icon_name) as icon_name,
  COALESCE(fb.budgeted, 0) as budgeted,
  COALESCE(fs.actual_spent, 0) as actual_spent,
  CASE 
    WHEN COALESCE(fb.budgeted, 0) > 0 THEN (COALESCE(fs.actual_spent, 0) / fb.budgeted) * 100
    ELSE 0
  END as usage_percentage,
  COALESCE(fb.budgeted, 0) - COALESCE(fs.actual_spent, 0) as remaining,
  CASE 
    WHEN COALESCE(fb.budgeted, 0) = 0 THEN 'no_budget'
...
FULL OUTER JOIN family_spending fs ON 
...
CREATE INDEX idx_mv_family_budget_category ON mv_family_budget_performance(category_id);
...
DROP TRIGGER IF EXISTS trigger_refresh_budget_on_orcamentos ON orcamentos;
...
DROP TRIGGER IF EXISTS trigger_refresh_budget_on_transactions ON transactions;
